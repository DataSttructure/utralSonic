...
u8 sim800c_call_test(void)
{
	...
	u8 cmode=0;	//模式
	            ...
	            //2:通话中
				      //3:接收到来电 
	LCD_Clear(WHITE);
	if(sim800c_send_cmd("AT+CTTSRING=0","OK",200))return 1;	//设置TTS来电设置
	if(sim800c_send_cmd("AT+CTTSPARAM=20,0,50,70,0","OK",200))return 1;	//设置TTS声音配置
...
u8 sim800c_sms_test(void)
{
	...
	if(sim800c_send_cmd("AT+CTTSPARAM=5,0,51,61,0","OK",200))return 1;	//设置TTS声音配置
...
u8 sim800c_spp_mode(u8 mode)
{
	  ...
	  Show_Str(40,60,200,16,"KEY_UP:返回上一级",16,0);
...
void sim800c_GB_ui(void)
{
	  ...
	  Show_Str(40,40+25,200,16,"请选择:",16,0); 	 	
